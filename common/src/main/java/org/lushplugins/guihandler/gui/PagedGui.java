...
public interface PagedGui<T> {
    ...
    @SlotIconProvider('>')
    default ItemStack nextPageIcon(SlotContext context) {
        Gui gui = context.gui();
    ...
    @SlotActionProvider('>')
    default void nextPageAction(Gui gui) {
        List<Slot> slots = gui.slots(slot -> slot.label() == getContentLabel());
        int pageSize = slots.size();
    ...
    @SlotIconProvider('<')
    default ItemStack previousPageIcon(SlotContext context) {
        return getPreviousPageIcon(context, context.gui().page() > 1);
    }

    @SlotActionProvider('<')
    default void previousPageAction(Gui gui) {
        if (gui.page() > 1) {
            gui.previousPage();
        }
    }
